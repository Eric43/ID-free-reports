...
st_mort <- st %>% select (c(date, deaths)) %>%
    group_by(date)%>%
    tally(deaths)
...
After a basic model for total cases, a mortality model can be developed.

```{r, Random Walk Mortality Forecasting model, echo=FALSE}


stmort_ts <- ts(st_mort[,2], start = c(year(start_cases), yday(start_cases)),
               end = c(year(end_cases), yday(end_cases)), frequency = 365)


autoplot(stmort_ts) +
  autolayer(rwf(stmort_ts, h=10),
    series="RWF Naïve", PI=FALSE) +
  autolayer(rwf(stmort_ts, drift=TRUE, h=10),
    series="RWF Drift", PI=FALSE) + 
  ggtitle(paste("Forecasts for COVID-19 mortality over next 10 days", state2select)) +
  xlab("Time") + 
  ylab("Number cases") +
  guides(colour=guide_legend(title="Forecast"))
...
##### The following section has not been double checked.  Need to verify numbers.
##### In addition need to develop a model for CDF and where on infect curve.

```{r, Basic PHR calculations very rough need probabilty modeling, echo = FALSE}

p_flu_pan <- 500000000/1800000000

tot_cases <- max(stcases_ts) #### Double check sum or max.


tot_dead <- max(stmort_ts)   ### Need to double check
...
March 1, 2020 Basic document created.
April 14, 2020 Automated document framework 
May 6, 2020 state_ID_report.Rmd sections finalized
May 21, 2020 Added basic references and EPS models.
July 17, 2020 Working on adding mortality model
July 23, 2020 changed the caluclation for determining total N 